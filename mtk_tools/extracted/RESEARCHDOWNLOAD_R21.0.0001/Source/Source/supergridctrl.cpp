	...
	if(lpDrawItemStruct->itemAction == ODA_DRAWENTIRE)
	{
		...
		for(int nColumn=1; GetColumn(nColumn, &lvc); nColumn++)
		{
			...
			if (rcItem.left < rcClipBox.right && rcItem.right > rcClipBox.left && rcItem.right > rcItem.left)
			{
				...
				clf = pItemInfo->GetBkColor(nColumn-1);
				if(clf!=MAXDWORD)
				{	
					CBrush br(clf);
...
void CSuperGridCtrl::DrawComboBox(CDC* pDC, CTreeItem *pSelItem, int nItem, int nColumn)
{
	...
	{
		if(ctrlType == CItemInfo::combobox) //lint !e1015
		{
			CRect rect;
...
void CSuperGridCtrl::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	int nItemHeight = tmc.tmHeight + tmc.tmExternalLeading;
				...
				{	
					...
					if(nIndex !=-1)
					{
						...
						if(pItem!=NULL)
						{
							...
							pInfo->SetCheck(!pInfo->GetCheck());//lint !e730
						}
					}
				}
			}break;
...
BOOL CSuperGridCtrl::HitTestOnSign(const CPoint &point, LVHITTESTINFO& ht)
{
	ht.pt = point;
	...
	if(ht.iItem!=-1)
	{
		...
		if(pItem!=NULL)
		{
			if(bHit)//if checkbox
			{
				...
				SetCheck(ht.iItem,!GetCheck(ht.iItem));//lint !e730
				CItemInfo *pInfo = GetData(pItem);
				pInfo->SetCheck(!pInfo->GetCheck());//lint !e730
			}
...
void CSuperGridCtrl::OnBegindrag(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(pNMListView->iItem != -1)
	{
		...
		if(pDragImage)
		{
			...
			/*pDragImage->*/CImageList::DragEnter(this, pNMListView->ptAction);
			SetCapture();
			m_bIsDragging = TRUE;
		}
		delete pDragImage;
...
{
//	OutputDebugString( L"In OnMouseMove\n" );
    if(m_bIsDragging)
    {
...
//		CListCtrl::OnLButtonDown(pMsg->wParam,pt);
//		OnLButtonDown(pMsg->wParam,pt);
//		
//		return 1;
// 	}
...
CSuperGridCtrl::CTreeItem* CSuperGridCtrl::InsertItem(CTreeItem *pParent, CItemInfo* lpInfo,  BOOL bUpdate)
{
	...
	else
	{
		...
		{
		   CString strTemp = GetData(pItem)->GetSubItem(i);
		   lvItem.mask = LVIF_TEXT;
		   lvItem.iSubItem = i+1;
		   lvItem.pszText = strTemp.GetBuffer(1);
		   SetItem(&lvItem);
		}
...
int CSuperGridCtrl::Expand(CTreeItem* pSelItem, int nIndex)
{
	if(ItemHasChildren(pSelItem) && IsCollapsed(pSelItem))
	{
		...
		while(pos != NULL)
		{
			...
			LV_ITEM lvItemTemp;
			lvItemTemp.mask = LVIF_TEXT | LVIF_INDENT | LVIF_PARAM;
			lvItemTemp.pszText =str.GetBuffer(1); 
			lvItemTemp.iItem = nIndex + 1;
			lvItemTemp.iSubItem = 0;
			lvItemTemp.lParam=(LPARAM)pNextNode;
			lvItemTemp.iIndent = GetIndent(pSelItem)+1;
			CListCtrl::InsertItem(&lvItemTemp);
			...
			for(int i=0; i< nSize;i++)
			{
			   CString strTemp=GetData(pNextNode)->GetSubItem(i);
			   lvItemTemp.mask = LVIF_TEXT;
			   lvItemTemp.iSubItem = i+1;
			   lvItemTemp.pszText=strTemp.GetBuffer(1);
			   SetItem(&lvItemTemp);
			}
			nIndex++;
		}
...
void CSuperGridCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	ht.pt = point;
	SubItemHitTest(&ht);
	BOOL bRet=OnItemLButtonDown(ht);
	if(bRet)
	{
		BOOL bSelect=TRUE;
		...
		if(bSelect && ht.iItem!=-1)
		{
			...
			MakeColumnVisible(Header_OrderToIndex(pHeader->m_hWnd, m_CurSubItem));
//			OutputDebugString( L"Enter OnLButtongDown\n" );
			CListCtrl::OnLButtonDown(nFlags, point);
			OnControlLButtonDown(nFlags, point, ht);

			CTreeItem * pTreeItem = GetTreeItem(ht.iItem);
...
{
	return (pItem == NULL)? NULL:pItem->m_pParent;
};
...
{
	return (pItem == NULL)? NULL:pItem->m_lpNodeInfo;
}
...
BOOL CRectangle::HitTest(const CPoint &pt)
{
